...
// The 2 render path options, in a radio button group at the right of the screen
enum JOBALLOCATION_TYPE
{
	JOBALLOCATION_SEQUENTIAL_JOB,
	JOBALLOCATION_INTERLEAVED_JOB,
	JOBALLOCATION_JOB_SHARING
};
...
bool compare(const JobQueueEntry& a, const JobQueueEntry& b)
{
...
#define IDC_TOGGLEWIRE                          3
#define IDC_TOGGLELIGHTVIEW                     4

#define IDC_JOBALLOCATION_GROUP                 5
#define IDC_JOBALLOCATION_SEQUENTIAL_JOB	    6
#define IDC_JOBALLOCATION_INTERLEAVED_JOB		7
#define IDC_JOBALLOCATION_JOB_SHARING			8

#define IDC_JOB_SORT							9

#define IDC_JOB_VARIANCE_RATETEXT				11
#define IDC_JOB_VARIANCE_RATE					12
...
const int                   g_iMaxThreads = 32;   // For true scalability, this should not be fixed at compile-time 
int                         g_iNumWorkThreads;    // One thread per physical processor, minus the main thread
HANDLE                      g_hWorkThreads[g_iMaxThreads];
HANDLE                      g_hBeginWorkThreadEvents[g_iMaxThreads];	// replace g_hBeginPerChunkRenderDeferredSemaphore[g_iMaxPerChunkRenderThreads]
HANDLE                      g_hEndWorkThreadEvents[g_iMaxThreads];
ID3D11DeviceContext*        g_pd3dWorkDeferredContexts[g_iMaxThreads] = {NULL};
ID3D11CommandList*          g_pd3dWorkCommandLists[g_iMaxThreads] = {NULL};
int                         g_iWorkThreadInstanceData[g_iMaxThreads];
ID3D11DeviceContext*        g_pd3dMainDeferredContext = NULL;
ID3D11CommandList*          g_pd3dMainCommandList = NULL;

int							g_iJobVarianceRate = 0; //levels from 0 to 10
unsigned int				g_iTotalNumJobs = 0;
volatile long				g_iJobIndex = 0;
JobQueueEntry				g_JobQueue[1024];
int							g_iJobGroupSize = 0;
bool						g_bSortJobs=false;

// The default render pathway
JOBALLOCATION_TYPE          g_iJobAllocationType = JOBALLOCATION_JOB_SHARING;
...
SceneParamsStatic			*g_pSceneStaticParams;
SceneParamsDynamic          *g_pSceneParamsDynamic;
...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
    ...
    DXUTCreateWindow( L"MultithreadedRendering11 - RemoveSemaphores" );
...
void InitApp()
{
	...
	g_HUD.AddRadioButton(IDC_JOBALLOCATION_SEQUENTIAL_JOB, IDC_JOBALLOCATION_GROUP, L"Sequential Job", 0, iY += iYo + 10, 170, 22);
	g_HUD.AddRadioButton(IDC_JOBALLOCATION_INTERLEAVED_JOB, IDC_JOBALLOCATION_GROUP, L"Interleaved Job", 0, iY += iYo, 170, 22);
	g_HUD.AddRadioButton(IDC_JOBALLOCATION_JOB_SHARING, IDC_JOBALLOCATION_GROUP, L"Job Sharing", 0, iY += iYo, 170, 22);
	CDXUTRadioButton* pRadioButton = g_HUD.GetRadioButton(IDC_JOBALLOCATION_JOB_SHARING);
	pRadioButton->SetChecked(true);

	WCHAR desc[256];
	swprintf_s(desc, L"Job Variance Rate: %d ", g_iJobVarianceRate);
	g_HUD.AddStatic(IDC_JOB_VARIANCE_RATETEXT, desc, 0, iY += iYo+10, 30, 10);
	g_HUD.AddSlider(IDC_JOB_VARIANCE_RATE, 0, iY += iYo, 150, 22, 0, 10, g_iJobVarianceRate);

	g_HUD.AddCheckBox(IDC_JOB_SORT, L"Sorting Jobs", 0, iY += iYo+10, 170, 22, g_bSortJobs);
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
        ...
        case IDC_TOGGLEWIRE:
			...
			break;
		case IDC_JOBALLOCATION_INTERLEAVED_JOB:
			g_iJobAllocationType = JOBALLOCATION_INTERLEAVED_JOB;
			break;
		case IDC_JOBALLOCATION_SEQUENTIAL_JOB:
			g_iJobAllocationType = JOBALLOCATION_SEQUENTIAL_JOB;
			break;
		case IDC_JOBALLOCATION_JOB_SHARING:
			g_iJobAllocationType = JOBALLOCATION_JOB_SHARING;
			break;
		case IDC_JOB_VARIANCE_RATE:
			{
				g_iJobVarianceRate = g_HUD.GetSlider(IDC_JOB_VARIANCE_RATE)->GetValue();
				WCHAR desc[256];
				swprintf_s(desc, L"Job Variance Rate: %d", g_iJobVarianceRate);
				g_HUD.GetStatic(IDC_JOB_VARIANCE_RATETEXT)->SetText(desc);
			}
			break;
		case IDC_JOB_SORT:
			g_bSortJobs = !g_bSortJobs; break;
			break;
    }
...
HRESULT InitializeWorkerThreads( ID3D11Device* pd3dDevice )
{
    ...
    g_iNumWorkThreads = GetPhysicalProcessorCount() - 2; // minus main thread and driver thread 

    // Restrict to the max static allocation --- can be easily relaxed if need be
	g_iNumWorkThreads = min(g_iNumWorkThreads, g_iMaxThreads );

    // Need at least on worker thread, even on a single-core machine
	g_iNumWorkThreads = max(g_iNumWorkThreads, 1 );
	...
	V_RETURN(pd3dDevice->CreateDeferredContext(0 /*Reserved for future use*/,
		&g_pd3dMainDeferredContext));

    for ( int iInstance = 0; iInstance < g_iNumWorkThreads; ++iInstance )
    {
		g_iWorkThreadInstanceData[iInstance] = iInstance;

		g_hBeginWorkThreadEvents[iInstance] = CreateEvent(NULL, FALSE, FALSE, NULL);
		g_hEndWorkThreadEvents[iInstance] = CreateEvent( NULL, FALSE, FALSE, NULL );

        V_RETURN( pd3dDevice->CreateDeferredContext( 0 /*Reserved for future use*/, 
            &g_pd3dWorkDeferredContexts[iInstance] ) );

		g_hWorkThreads[iInstance] = ( HANDLE )_beginthreadex( NULL,
            0, 
            _PerChunkRenderDeferredProc, 
            &g_iWorkThreadInstanceData[iInstance],
            CREATE_SUSPENDED, 
            NULL );
        ...
        DXUT_SetDebugName(g_pd3dWorkDeferredContexts[iInstance], threadid );
#endif

        ResumeThread(g_hWorkThreads[iInstance] );
    }
...
void RenderMesh( CMultiDeviceContextDXUTMesh* pMesh, 
                ...
                UINT iSpecularSlot )
{
	g_JobQueue[g_iTotalNumJobs].iMesh = iMesh;
	g_JobQueue[g_iTotalNumJobs].iWeight = ceil(((double)rand() / (double)RAND_MAX) * g_iJobVarianceRate);
...
// Build Command Lists
//--------------------------------------------------------------------------------------
void BuildCommandList(const int iBegin, const int iEnd, ID3D11DeviceContext* pd3dDeferredContext, ID3D11CommandList*& pd3dCommandList)
{
	HRESULT hr;
	...
	V(RenderSceneSetup(pd3dDeferredContext, g_pSceneStaticParams, g_pSceneParamsDynamic));

	if (g_iJobAllocationType == JOBALLOCATION_INTERLEAVED_JOB)
	{
		for (int i = iBegin; i < iEnd; i += (g_iNumWorkThreads + 1))
		{
			// Call dummy function to add random load
			g_Mesh11.RenderMesh(g_JobQueue[i].iWeight,
				false,
				NULL,
				0,
				1,
			...
			// Render the mesh of a job in immediate context
			RenderMeshDirect(pd3dDeferredContext, g_JobQueue[i].iMesh);
		}
	}
	else if (g_iJobAllocationType == JOBALLOCATION_SEQUENTIAL_JOB)
	{
	...
	{
		unsigned int index = InterlockedIncrement((LPLONG)&g_iJobIndex) - 1;
		while (index < iEnd)
		{
			// Call dummy function to add random load
			g_Mesh11.RenderMesh(g_JobQueue[index].iWeight,
				false,
				NULL,
				0,
				1,
			...
			// Render the mesh of a job in immediate context
			RenderMeshDirect(pd3dDeferredContext, g_JobQueue[index].iMesh);
			index = InterlockedIncrement((LPLONG)&g_iJobIndex) - 1;
		}
	}

	// Finish command list
	V(pd3dDeferredContext->FinishCommandList(!g_bClearStateUponFinishCommandList,
		&pd3dCommandList));
}


//--------------------------------------------------------------------------------------
// Render the scene from a deferred context in the main thread, 
// or worker threads, handling objects alternately
                    ...
                    const SceneParamsDynamic *pDynamicParams )
{
	...
	if(g_bSortJobs) std::sort(g_JobQueue, g_JobQueue + g_iTotalNumJobs, compare);

	if (g_iJobAllocationType == JOBALLOCATION_SEQUENTIAL_JOB)
	{
		g_iJobGroupSize = ceil(g_iTotalNumJobs / (g_iNumWorkThreads + 1));
	}

    // Signal all worker threads to finalize their command lists
    for ( int iInstance = 0; iInstance < g_iNumWorkThreads; ++iInstance )
    {
		SetEvent(g_hBeginWorkThreadEvents[iInstance]);
    }

	if (g_iJobAllocationType == JOBALLOCATION_SEQUENTIAL_JOB)
	{
		BuildCommandList(0, g_iJobGroupSize, g_pd3dMainDeferredContext, g_pd3dMainCommandList);
	}
	else
	{
		BuildCommandList(0, g_iTotalNumJobs, g_pd3dMainDeferredContext, g_pd3dMainCommandList);
	}

	// Execute Main command list.
	pd3dContext->ExecuteCommandList(g_pd3dMainCommandList,
		!g_bClearStateUponExecuteCommandList);
	SAFE_RELEASE(g_pd3dMainCommandList);

	// Wait until all worker threads signal that their command lists are finalized
	WaitForMultipleObjects(g_iNumWorkThreads,
		g_hEndWorkThreadEvents, TRUE, INFINITE);

    // Execute all work command lists.  Note these now produce a scattered render order.
    for ( int iInstance = 0; iInstance < g_iNumWorkThreads; ++iInstance )
    {
        pd3dContext->ExecuteCommandList(g_pd3dWorkCommandLists[iInstance],
            !g_bClearStateUponExecuteCommandList );
        SAFE_RELEASE(g_pd3dWorkCommandLists[iInstance] );
    }
...
VOID RenderShadow( int iShadow, ID3D11DeviceContext* pd3dContext )
{
	...
	g_pSceneStaticParams = &g_StaticParamsShadow[iShadow];
	g_pSceneParamsDynamic = &DynamicParams;
...
VOID RenderMirror( int iMirror, ID3D11DeviceContext* pd3dContext )
{
	...
	g_pSceneStaticParams = &g_StaticParamsMirror[iMirror];
	g_pSceneParamsDynamic = &DynamicParams;
...
VOID RenderSceneDirect( ID3D11DeviceContext* pd3dContext )
{
	...
	g_pSceneStaticParams = &g_StaticParamsDirect;
	g_pSceneParamsDynamic = &DynamicParams;
...
unsigned int WINAPI _PerChunkRenderDeferredProc( LPVOID lpParameter )
{
    ...
    ID3D11DeviceContext* pd3dDeferredContext = g_pd3dWorkDeferredContexts[iInstance];
    ID3D11CommandList*& pd3dCommandList = g_pd3dWorkCommandLists[iInstance];

    for (;;)
    {
        // Wait for a work queue entry
		WaitForSingleObject(g_hBeginWorkThreadEvents[iInstance], INFINITE);
		...
		{
			iBegin = iInstance + 1;
			iEnd = g_iTotalNumJobs;
		}
		else if (g_iJobAllocationType == JOBALLOCATION_SEQUENTIAL_JOB)
		{
			iBegin = (iInstance + 1)*g_iJobGroupSize;
			iEnd = min(iBegin+g_iJobGroupSize, g_iTotalNumJobs);
		}
		else
		{
			iBegin = 0;
		...
		BuildCommandList(iBegin, iEnd, pd3dDeferredContext, pd3dCommandList);

		// Tell main thread command list is finished
		SetEvent(g_hEndWorkThreadEvents[iInstance]);
    }
}
                                  ...
                                  float fElapsedTime, void* pUserContext )
{
    ...
    {
		// generate the same random numbers for each frame
		srand(1);
...
void CALLBACK OnD3D11DestroyDevice( void* pUserContext )
{
	SAFE_RELEASE(g_pd3dMainDeferredContext);

	for ( int iInstance = 0; iInstance < g_iNumWorkThreads; ++iInstance )
    {
        CloseHandle(g_hWorkThreads[iInstance] );
        CloseHandle(g_hEndWorkThreadEvents[iInstance] );
		CloseHandle(g_hBeginWorkThreadEvents[iInstance] );
        SAFE_RELEASE(g_pd3dWorkDeferredContexts[iInstance] );
    }
